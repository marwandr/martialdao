            ...
            if addr not in clients:
                ...
                for client in clients:
                    try:
                        if client != addr:
                            try:
                                if data.decode().startswith("FIGHTER-") or data.decode().startswith("CONNECT_REQ:") or "RESET" in data.decode():
                                    pass
                                else:
                            ...
                            try:
                                ...
                                elif "RESET" in data.decode():
                                    playername0 = players[0].playerID
                                    playername1 = players[0].playerID
                                    players[0] = Cultivator(300, 310, False, SPRITE_DATA, playername0)
                                    players[1] = Cultivator(700, 310, True, SPRITE_DATA, playername1)
                                    server.sendto("RESET-SUCCESS".encode(), addr)
                            except:
                                pass
                    except:
